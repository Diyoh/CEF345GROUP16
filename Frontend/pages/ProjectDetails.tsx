...
import { ProjectStatus, Comment } from '../types';

export const ProjectDetails: React.FC = () => {
  const { id } = useParams<{ id: string }>();
  const { projects, comments, addComment } = useAppStore();
  const project = projects.find(p => p.id === id);

  // Comment State
  const [commentText, setCommentText] = useState('');
  const [authorName, setAuthorName] = useState('');
  const [authorType, setAuthorType] = useState<'Citizen' | 'NGO'>('Citizen');
  ...
  const toggleExpandImages = (commentId: string) => {
